let log = console.log;
const tdClientId = '0YLHPDL4BGVRRFVU2D9QMXG3DTKHEIJB';
const tdRedirectUrl = `http://localhost:3000`;
...
async function callTD(url: string, tokens: any = null) {
  ...
  if (!tdClientId) throw new Error(`don't have tdClientID in callTD`);
...
async function refreshTheToken2(refreshToken: any) {
  ...
  if (!tdClientId) throw new Error(`don't have tdClientID in refreshTheToken2`);
  var params = {
    body:
      'grant_type=refresh_token&refresh_token=' +
      encodeURIComponent(refreshToken) +
      '&access_type=offline&client_id=' +
      encodeURIComponent(tdClientId) +
      '&redirect_uri=' +
      encodeURIComponent(tdRedirectUrl),
