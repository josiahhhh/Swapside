import { Frequency, Result, Strategy } from "typings/typings";
/*
...
const ratePerCompound = (
  annualInterestRate: number,
  compoundFrequency: Frequency
): number => annualInterestRate / frequencyPerYear(compoundFrequency);

const sumDepositsForPeriod = (
  amount: number,
...
export const calculateCompoundInterest = (strategy: Strategy) => {
  ...
  for (let year: number = 0; year < strategy.numberOfYears; year++) {
    ...
    for (
      let period: number = 0;
      period < frequencyPerYear(compoundFrequency);
      period++
    ) {
      // For each compound period in the year (1 or 12)

      // Work out deposits this period
      const depositThisPeriod = sumDepositsForPeriod(
        regularDeposit,
        depositFrequency,
        compoundFrequency
      );
